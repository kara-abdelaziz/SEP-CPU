...
; Fibonacci program, it display fibonacci suite terms
...
count_var:  ; count_var declaration
	#res 1

term_0:     ; term_0 declaration
	#res 1
	
term_1:     ; term_1 declaration
	#res 1
	
term_2:     ; term_2 declaration
	#res 1
...
loop:	
	ld   (term_0)
	add  (term_1)
	str  (term_2)  ; term_2 <- term_0 + term_1
	
	out            ; display term_2 for the user
	
	ld   (term_1)
	str  (term_0)  ; term_0 <- term_1
	
	ld   (term_2)
	str  (term_1)  ; term_1 <- term_2
	
	ld   (count_var)
	sub  1
	str  (count_var) ; count_var--
	
	cmp  0
	bne  loop        ;  loop while (count_var != 0)
